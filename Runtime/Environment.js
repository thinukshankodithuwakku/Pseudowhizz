...
import { MK_CHAR, MK_BOOL, MK_NATIVE_FN, MK_NULL, MK_NUMBER, MK_STRING } from "./Value.js";
import { conv_memex_to_val, adjust_index, isint, Confirm } from "./Eval/Expressions.js";
import { errorLog, makeError, FileValues, initial_frame } from "../Main.js";
...
export function arityCheck(expected, got, ln) {
    if (expected != got) {
...
export function SetupGlobalScope() {
    ...
    env.declareVar("LENGTH", MK_NATIVE_FN((args, scope) => {
        ...
        if (subjectComponent.type == "Object") {
            ...
            const func = env.lookupVar("LENGTH", subjectComponent.ln, [initial_frame], env);
    ...
    env.declareVar("NUM_TO_STR", MK_NATIVE_FN((args, scope) => {
        arityCheck(1, args.length, args[0].ln);
        if (errorLog.length > 0) {
            return MK_NULL();
        }
    ...
    env.declareVar("STR_TO_NUM", MK_NATIVE_FN((args, scope) => {
        arityCheck(1, args.length, args[0].ln);
        if (errorLog.length > 0) {
            return MK_NULL();
        }
        if (localStorage.getItem('Af') == "true") {
            ...
            return MK_NUMBER(parseFloat(args[0].value), "Auto");
        }
        else {
    ...
    env.declareVar("EOF", MK_NATIVE_FN((args, scope) => {
        arityCheck(1, args.length, args[0].ln);
        if (errorLog.length > 0) {
            return MK_NULL();
        }
...
export default class Environment {
    ...
    declareVar(varname, value, isConstant, env, StackFrame) {
    ...
    assignVar(varname, value, ln, StackFrames, callEnv) {
    ...
    lookupVar(varname, ln, StackFrames, callEnv) {
        const env = this.resolve(varname, ln, StackFrames, callEnv);
        if (errorLog.length > 0) {
            return MK_NULL();
        }
        const lookup = env.variables.get(varname);
        return lookup;
    }
    resolve(varname, ln, StackFrames, env) {
