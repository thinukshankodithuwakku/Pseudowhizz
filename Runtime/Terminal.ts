...
export default class Terminal {
    ...
    private avoid = ["help(copyright)","help(credits)","help()", "help(help)", "help(keywords)", "help(symbols)", "help(modules)", "help(quit)"];
    private block_descs = new Map<string, string>();
    ...
    private race<T>(promise: Promise<T>, ms: number): Promise<T> {
        let timer: number;
    ...
    private split_into_collumns(list : string[], colHeight : number) : string[][] {

        while(list.length % colHeight != 0) list.push('');
    ...
    private cancel(msg : string){
        ...
        const wrapper = this.console.children[this.console.children.length - 1] as HTMLElement;
        const line = wrapper.children[1] as HTMLInputElement;
    ...
    private async main(msg : string) {

        let res = null;

        try {

            res = (await this.race(this.ask(msg), this.timeout)).trim() as string;
    ...
    private process_help_request(res : string) {
    ...
    private async help_handler(msg : string) {

        let res = null;

        try {

            res = (await this.race(this.ask(msg, 'help>'), this.timeout)) as string;
        ...
        else this.process_help_request(res);
    ...
    private throw_err(msg : string, kind : "RuntimeError" | "SyntaxError" | "TypeError" | "NameError") {
    ...
    public async ask(msg : string, chev = '>') : Promise<string> {
    ...
    private async std_in(chev : string = '>') : Promise<string> {
