...
const FnArgDesc : Record<string, string[]> = {
...
export function save_user_fn(line : string) {

    
    const at = () => { return ln_tokens[0] ? (ln_tokens[0].type as Tokens) : Tokens.Null}


    const ln_tokens = tokenize_line(line, 0);

    
    if(!(at() == Tokens.Function || at() == Tokens.Procedure)) return;
    const is_fn = ln_tokens.shift().type == Tokens.Function; //Consume FUNCTION
    


    if(at() != Tokens.Identifier) return;
    const name = ln_tokens.shift().value; //Consume name

    if([...UserFnArgDesc.keys()].includes(name)) return;
    


    if(at() != Tokens.OpenBracket) return;
    ...
    while(!end()){
        ...
        while(at() != Tokens.Colon){
            ...
            if(at() == Tokens.Identifier) {names.push(ln_tokens.shift().value)}
            
            if(at() == Tokens.Comma) {ln_tokens.shift();}
            else if(at() != Tokens.Colon && at() != Tokens.Identifier){return}

        }

        

        if(at() != Tokens.Colon) return;
        ln_tokens.shift();

        if(!dataTypes.includes(at())) return;

        if(at() == Tokens.Array){

            ln_tokens.shift();
            if(at() != Tokens.Of) return;
            ln_tokens.shift();
            if(at() == Tokens.Array || !dataTypes.includes(at())) return;
            dataType = `ARRAY OF ${ln_tokens.shift().value}`;

        }
        else dataType = ln_tokens.shift().value;
        

        names.forEach(name => params.set(name, dataType));

        
        if(!end() && at() != Tokens.Comma) return;
        if(at() == Tokens.Comma) ln_tokens.shift();
    ...
    if(at() != Tokens.CloseBracket) return;
    
    ln_tokens.shift();
    if(is_fn && at() != Tokens.Returns) return;
    else if(!is_fn && at() == Tokens.Returns) return;
    ln_tokens.shift();

    let returnType = null;
    
    if(is_fn && (!at() || !dataTypes.includes(at()))) return;
    ...
    else returnType = is_fn ? ln_tokens[0].value : undefined;

    if(is_fn && !(name && returnType)) return;
...
function spit_html(func_name : string, current_parameter : number) : {bef: string, hl: string, aft : string}{
...
function userFn_spit_html(name : string, param : number) : string {
...
export class DescBox {
    ...
    private descBoxBody : HTMLDivElement;
    ...
    private scan(fn_name : string) {

        for(const name of natives) if(fn_name.endsWith(name)) return name;

        return null;

    }

    public user_tick(name : string, co_ords, parameter : number = 0) {
    ...
    public tick(fn_name : string, co_ords, parameter? : number) {
        ...
        if(parameter || parameter == 0){
            ...
            if(name != "RANDOM"){
                document.getElementsByClassName('desc-box-body')[0].appendChild(
                    document.createTextNode(FnArgDesc[name][parameter])
                );
                document.getElementsByClassName('desc-box-body')[0].appendChild(
            ...
            document.getElementsByClassName('desc-box-body')[0].appendChild(
                document.createTextNode(FnDescRec[name])
            );
    ...
    public countParameters(line : string, co_ords, offset : number, source : 'comma' | 'bracket' | 'arrow' | 'backspace') {
        ...
        if(double_quotes % 2 == 1 && source != "arrow") return;



        chars.pop();

        if(chars.join('').endsWith('//') && source != "arrow") return;
        else if(chars.join('').endsWith("'") && single_quotes % 2 == 1 && source != "arrow") return;
        ...
        while(i >= 0){
            ...
            if(tokens[i].type == Tokens.CloseBracket) bracs--
            else if(tokens[i].type == Tokens.OpenBracket) bracs++
        ...
        while(i >= 0){
            ...
            if(at.type == Tokens.CloseBracket) bracs--;
            else if(at.type == Tokens.OpenBracket) bracs++;
            else if(at.type == Tokens.CloseSquareBracket) sqrBracs--;
            else if(at.type == Tokens.OpenSquareBracket) sqrBracs++;
            else if(at.type == Tokens.Comma && bracs == 0 && sqrBracs == 0) currentParameter++;
...
export function select_all(editor, selection : SelectionOrigin) {
    ...
    if(!word || word.length == 0 || word.trim() == "") return;
...
export function highlight_all_tokens(editor, curTkMarks, token : cm_token){
    ...
    for (let line = 0; line < editor.lineCount(); line++) {
        ...
        for(let i = 0; i < tokens.length; i++){
            ...
            if(token.string === string && highlight){
                ...
                if(names.includes(token.type)){
                    ...
                    switch(token.type){
                        ...
                        case "variable":
                            ...
                            if(nxtToken.string == '<--' || nxtToken.string == "←") className = "cm-token-def";
                            else{
                                ...
                                if(ln.length >= 4 && (ln.map(tk => tk.value).includes('FUNCTION') || ln.map(tk => tk.value).includes('PROCEDURE')) && (ln[0].value == 'FUNCTION' || ln[0].value == 'PROCEDURE')){
                                    ...
                                    if(ln[1].type == Tokens.Identifier && ln[2].type == Tokens.OpenBracket && ln[3].value == token.string) isFirstParam = true;

                                }

                                if(isFirstParam) className = "cm-token-def";

                            }
                            break;
                            
                        case "constant":
                            if(prevToken.string == 'CONSTANT' && (nxtToken.string == '<--' || nxtToken.string == "←")) className = "cm-token-def";
                            break;

                        case "userFn":
                            if(prevToken.string == 'FUNCTION' || prevToken.string == 'PROCEDURE') className = "cm-token-def";
                            break;
                ...
                else className = "cm-cursor-token";
